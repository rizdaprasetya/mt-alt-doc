(function () {
  // @modified from https://cdn.jsdelivr.net/npm/docsify@4.11.6/lib/plugins/search.js
  ...
  function genIndex(path, content, router, depth) {
    ...
    tokens.forEach(function (token) {
      if (token.type === 'heading' && token.depth <= depth) {
        ...
        // @modified
        if(index[slug].body && index[slug].body.replace){
          index[slug].body = index[slug].body
  ...
  function init(config, vm) {
    ...
    // var once = 0; isExpired = once ? 0 : 1; once = 1; console.log('rebuilding search index');
